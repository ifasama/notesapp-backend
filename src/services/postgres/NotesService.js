...
class NotesService {
  ...
  async addNote({
    ...
    // to check whether the insertion is successful
    if (!result.rows[0].id) {
      throw new InvariantError('Catatan gagal ditambahkan');
    }

    // menghapus cache bila terjadi perubahan data
  ...
  async editNoteById(id, { title, body, tags }) {
    ...
    // mendefinisikan owner dan menghapus cache bila terjadi perubahan data
  ...
  async verifyNoteOwner(id, owner) {
    ...
    const noteOwner = result.rows[0].owner;
